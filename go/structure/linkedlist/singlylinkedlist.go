...
//NewSingly return a new instance of a linked list
...
//DelAtBeg  deletes the snode at the head(beginning) of  the list and returns its value. Return -1 if the list is empty.
...
func (ll *Singly) CheckRangeFromIndex(left, right int) error {
	if left > right {
		...
		return errors.New("right boundary can not be greater than the length of the linked list")
	}
